...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        sch = scheme_eval(first, env)
        check_procedure(sch)
        return scheme_apply(sch, rest.map(lambda x: scheme_eval(x, env)), env)
        # END PROBLEM 5
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('Length doesnt nmatch')
        forms = formals
        value = vals
        while forms is not nil:
            child.define(forms.first, value.first)
            forms, value = forms.second, value.second
        # END PROBLEM 11
        return child
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    formal, val, b = nil, nil, bindings
    while b is not nil:
        check_form(b.first, 2, 2)
        formal = Pair(b.first.first, formal)
        v = scheme_eval(b.first.second.first, env)
        val = Pair(v, val)
