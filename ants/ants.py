...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        def move(place):
            ...
            if place is hive or transitions > self.max_range:
                return None
            elif transitions < self.min_range:
                transitions += 1
                return move(place.entrance)
            elif place.bees:
                return random_or_none(place.bees)
            else:
                if place.entrance:
                    #print(place.bees, place.entrance)
                    transitions += 1
                    return move(place.entrance)
                else:
                    return None
        return move(self.place)
        # END Problem 3 and 4
...
# BEGIN Problem 8
class WallAnt(Ant):
    """ A WallAnt class that does nothing each turn and is very useful
    because it has a large armor"""
    name = 'Wall'
    food_cost = 4
    implemented = True
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower is subclass of Thrower Ant that is watersafe and can 
    be deployed in the water"""
    name = 'Scuba'
    implemented = True
    food_cost = 6
    watersafe = True
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.imposter: 
            ...
            existing_place = self.place.exit
            while existing_place is not None:
                if existing_place.ant and existing_place.ant not in self.ants_double:
                    self.ants_double.append(existing_place.ant)
                    existing_place.ant.damage = existing_place.ant.damage*2
                if isinstance(existing_place.ant, BodyguardAnt) \
                   and existing_place.ant.ant \
                   and existing_place.ant.ant not in self.ants_double:
                    existing_place.ant.ant.damage = existing_place.ant.ant.damage*2
                    self.ants_double.append(existing_place.ant.ant)
                existing_place = existing_place.exit
        # END Problem 13
